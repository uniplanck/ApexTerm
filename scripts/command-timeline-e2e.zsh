...
TMP_ROOT="$(mktemp -d /tmp/apexterm-command-timeline.XXXXXX)"
SUPPORT="$TMP_ROOT/support"
PROBE="$TMP_ROOT/command-timeline-probe.txt"
...
stop_app() {
  ...
  print -u2 -r -- "ApexTerm did not stop before Command Timeline E2E"
  return 1
}
...
APEXTERM_COMMAND_TIMELINE_PROBE_FILE="$PROBE" \
"$APP" >"$APP_LOG" 2>&1 &
APP_PID=$!
...
for _ in {1..1200}; do
  ...
  if ! kill -0 "$APP_PID" 2>/dev/null; then
    print -u2 -r -- "ApexTerm exited before Command Timeline probe completed"
...
if [[ ! -f "$PROBE" ]] || ! grep -Fqx 'probe_complete=1' "$PROBE"; then
  print -u2 -r -- "Command Timeline probe timed out"
  tail -80 "$APP_LOG" >&2 || true
  exit 1
fi
...
if (( ${#missing[@]} > 0 )); then
  print -u2 -r -- "Command Timeline probe assertions failed:"
  printf '  %s\n' "${missing[@]}" >&2
  print -u2 -r -- '--- PROBE ---'
  cat "$PROBE" >&2
  print -u2 -r -- '--- APP LOG ---'
...
print -r -- 'COMMAND_TIMELINE_UI_E2E=PASS'
cat "$PROBE"
